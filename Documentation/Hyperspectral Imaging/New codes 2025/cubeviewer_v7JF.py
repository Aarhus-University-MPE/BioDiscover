...
# Newly added shit JF to get scaling to work.
...
class App(tk.Tk):
    ...
    def plot_spectrum(self, roi_type: Literal["Custom", "Horizontal", "Vertical"] = "Custom"):
        ...
        # Find corresponding tab data
        for name, info in self.tabs.items():
            tab_index = self.notebook.index(selected_tab)
            tab_widget = self.notebook.tabs()[tab_index]
            if self.notebook.tab(tab_widget, "text") == name:
                datacube = info['data']
                channel_slider = info['channel_slider']
                ax = info['ax']
                break
        else:
            print("No datacube found for active tab.")
            return

        # Use current channel from the slider
        channel = int(channel_slider.get())
        height, width, channels = datacube.shape

        fig, ax = plt.subplots()

        if roi_type == "Vertical":
            center_x = width // 2
            slice_width = 10  # pixels
            vertical_slice = datacube[:, center_x - slice_width//2 : center_x + slice_width//2, channel]
            ax.imshow(vertical_slice, cmap='gray', aspect='auto')
            ax.set_title(f"Vertical ROI (x={center_x}) - Channel {channel}")

        elif roi_type == "Horizontal":
            center_y = height // 2
            slice_height = 10  # pixels
            horizontal_slice = datacube[center_y - slice_height//2 : center_y + slice_height//2, :, channel]
            ax.imshow(horizontal_slice, cmap='gray', aspect='auto')
            ax.set_title(f"Horizontal ROI (y={center_y}) - Channel {channel}")

        elif roi_type == "Custom":
            # Center crop box of 50x50 pixels
            center_y = height // 2
            center_x = width // 2
            half_box = 25
            crop = datacube[
            ...
            ax.set_title(f"Custom ROI (center crop) - Channel {channel}")
